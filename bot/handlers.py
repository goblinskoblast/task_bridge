...
def _build_welcome_message(is_first_auth: bool, pending_count: int) -> str:
    ...
    return (
        ...
        "• сохранить точку через /addpoint и потом писать запросы по стоп-листу, бланкам и мониторингам обычным сообщением"
    )
...
4. Для отчётов достаточно один раз сохранить точку, дальше можно писать запросы обычным текстом."""
