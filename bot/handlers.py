...
def _build_welcome_message(is_first_auth: bool, pending_count: int) -> str:
    ...
    return (
        ...
        f"• нажать «{PANEL_BUTTON_TEXT}» и открыть задачи\n"
        f"• нажать «{AGENT_MAIN_BUTTON_TEXT}» и написать запрос обычным сообщением\n"
        "• открыть поддержку, если нужна помощь\n"
        "• открыть агента, сохранить точку и дальше писать запросы по стоп-листу, бланкам и мониторингам обычным сообщением"
    )


def _build_help_message() -> str:
    return """❓ <b>Коротко о возможностях TaskBridge</b>

<b>Самый простой путь</b>
• внизу уже есть кнопки «📱 Панель задач» и «🤖 Агент»
• в агенте один раз подключите систему и добавьте точку
• лучше писать обычным текстом: <code>пришли стоп-лист по Сухой Лог, Белинского 40</code>
• <code>покажи бланки по всем добавленным точкам</code>
• <code>присылай бланки по Сухой Лог, Белинского 40 каждые 3 часа</code>
• <code>что у меня включено</code>
• <code>не присылай бланки по Сухой Лог, Белинского 40</code>

<b>Команды, если так удобнее</b>
• /start — обновить главное меню
...
4. Для отчётов и мониторинга достаточно один раз сохранить точку в агенте, дальше можно писать запросы обычным текстом."""
...
async def cmd_help(message: Message):
    ...
    await message.answer(_build_help_message(), parse_mode="HTML")
